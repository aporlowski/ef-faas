...
In this project we deploy and benchmark an AI service using Google’s function-as-a-service (FaaS) platform, Cloud Functions, and compare the performance to benchmarks conducted in [1] for traditional cloud virtual machines, IoT devices, and locally hosted containers. In recent work we use the Generalized AI Service (GAS) Generator to autogenerate and deploy a RESTful AI services using the cloudmesh-openapi utility [1]. The example service, EigenfacesSVM, is a facial recoginition example taken from Scikit-learn and modified to meet our needs [2].  The purpose of this project is to compare benchmark results of the service deployed in a FaaS model to the server-ful paradigms tested in the original work, and explain the development and deployment differences between cloudmesh-openapi and the FaaS platform.

## Background

### Autogenerated AI services with GAS

In [1] we adapted the EigenfacesSVM model from Scikit-learn to be a set of Python functions that download image data from a remote repository, trains the AI model, uploads an image for prediction, and predicts the label for the uploaded image. Cloudmesh-openapi automatically translates and deploys these Python functions into a RESTful service. We benchnmarked the AI functions on various platforms inclding cloud virtual machines from AWS, Google, and Azure, as well as Raspberry Pi platforms, a Mac book, and a docker container run on that MacBook. It is important to note that tehse services were depoyed in a server-ful manner, where the hosting platform is continuously running the service. In contrast, this projects aims to develop the same servcice in a serverless manner, where each invocatino of the service is potentially carried out by differenet hosting instance.

### Cloud Functions

## Development

### Cloudmesh Openapi Development
